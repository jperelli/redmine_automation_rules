...
# servers, all see the same requests). Adapted from Redmine Periodic Task.
class AutomationRulesSchedulerLock < (defined?(ApplicationRecord) ? ApplicationRecord : ActiveRecord::Base)
  ROW_ID = 1
