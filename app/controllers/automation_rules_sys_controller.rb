# Runs the scheduled rules checker on demand, for cron-less setups that call
...
class AutomationRulesSysController < ActionController::Base
  before_action :check_enabled

  def check
    count = AutomationRulesChecker.check!(source: 'endpoint')
    render plain: "Automation rules: #{count} rule(s) run", status: :ok
  end
